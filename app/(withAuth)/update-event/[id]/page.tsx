...
export default function EventForm({
  ...
  useEffect(() => {
    async function getEventData() {
      const eventData = await client.events.query.get(Number(id));
      if (eventData) {
        setValue("name", eventData.name);
        setValue("capacity", Number(eventData.capacity));
        if (eventData.dates) {
          setValue(
            "dates",
            eventData.dates.map((dateRange) => ({
              from: new Date(Number(dateRange[0])),
              to: new Date(Number(dateRange[1])),
            }))
          );
        }
        /* append({
  ...
  client.events.query.get(+id).then(console.log);
            ...
            <Stack spacing={3}>
              ...
              <Box>
                <TextField
                  ...
                  value={`${
                    control._formValues.dates[0]?.from.toLocaleDateString(
                      "en-US"
                    ) ?? ""
                  } - ${
                    control._formValues.dates[0]?.to.toLocaleDateString(
                      "en-US"
                    ) ?? ""
                  }`}
                    ...
                    >
                      <Stack spacing={2}>
                        <TextField
                          ...
                          helperText={
                            errors.ticketClasses?.[index]?.id?.message
                          }
                        />
                        <TextField
                          label="Description"
                          {...register(
                            `ticketClasses.${index}.description` as const
                          )}
